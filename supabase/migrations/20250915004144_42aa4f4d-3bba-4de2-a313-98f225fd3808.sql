-- IPGenie International — Supabase schema (Fixed - Handle Dependencies)
...
-- =========================
-- Helper: is_admin() for RLS (now that role column exists)
-- =========================
...
-- =========================
-- Update payments table (now that policies are dropped)
-- =========================
do $$
begin
  ...
  -- Remove user_id column as payments will be linked via filing_id (policies already dropped)
...
-- payments policies (NEW - secure: only via related filing ownership, no user_id)
